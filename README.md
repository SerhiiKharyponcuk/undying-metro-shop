...
В окне «Менеджеры» показываются два оператора с живым статусом. При переходе в Telegram выбранный менеджер атомарно помечается занятым в PostgreSQL на 10 минут, поэтому другой посетитель не сможет одновременно выбрать его. Статусы обновляются автоматически.
