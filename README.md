...
Отдельная панель `/admin/` позволяет модерировать отзывы, отвечать от имени магазина, искать и фильтровать заявки, менять их статусы и вести переписку. Для удобного входа также доступен короткий адрес `/admin.html`. Публичного или стандартного пароля нет.
