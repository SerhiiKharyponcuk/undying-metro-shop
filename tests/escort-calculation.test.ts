...
describe("escort financial calculation", () => {
  it("конвертирует оплату, отдаёт 3% создателю и делит остаток на троих", () => {
    ...
    expect(formatMinor(result.creatorAmountMinor)).toBe("150.00");
    expect(result.shares.map(formatMinor)).toEqual(["1616.67", "1616.67", "1616.66"]);
  });
  ...
  it("распределяет остаток копеек без потери денег", () => {
    ...
    expect(result.creatorAmountMinor + result.shares.reduce((sum, share) => sum + share, 0n)).toBe(result.amountUahMinor);
