...
describe("escort financial calculation", () => {
  it("конвертирует оплату, отдаёт 3% директору, 10% создателю и делит остаток на троих", () => {
    ...
    expect(formatMinor(result.directorAmountMinor)).toBe("150.00");
    expect(formatMinor(result.creatorAmountMinor)).toBe("500.00");
    expect(result.shares.map(formatMinor)).toEqual(["1450.00", "1450.00", "1450.00"]);
  });
  ...
  it("распределяет остаток копеек без потери денег", () => {
    ...
    expect(result.directorAmountMinor + result.creatorAmountMinor + result.shares.reduce((sum, share) => sum + share, 0n)).toBe(result.amountUahMinor);
