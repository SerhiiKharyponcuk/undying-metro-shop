...
Панель получает курсы EUR и USD из официального API НБУ по выбранной дате. Полученный курс сохраняется вместе с расчётом и позже не меняется. Если API НБУ временно недоступен, администратор может ввести курс к гривне вручную; источник курса также сохраняется в базе.

Миграции также создают журнал штрафов и связи замены сопровождающих. Баланс «Банка Metro Shop» вычисляется из неизменяемых записей штрафов, поэтому не требует отдельной переменной окружения или ручного начального значения.
