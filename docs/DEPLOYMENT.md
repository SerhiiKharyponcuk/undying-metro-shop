...
Пароль по умолчанию отсутствует. В Render Shell временно задайте `ADMIN_INITIAL_USERNAME` и `ADMIN_INITIAL_PASSWORD` (не короче 12 символов), затем выполните:

```bash
npm run admin:create
```

После успешного создания удалите `ADMIN_INITIAL_PASSWORD` и `ADMIN_INITIAL_USERNAME` из окружения. Пароль сохраняется только в виде Argon2id-хеша.
